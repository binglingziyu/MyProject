...
public class Binding {
	...
	public void Execute(View view, Object object) {
		try {
			for (Control item : mControls) {
				item.Handler.ToControl(view.findViewById(item.ID), object);
			}
		} catch (Exception e) {
	 ...
	 * @param xml
	 */
	public void Execute(Activity activity, Object object){
		Execute(activity.findViewById(android.R.id.content),object);
	}
	 ...
	 * @param xml
	 */
	public void Execute(Object holder,Object object){
		try {
			for(int i=0;i<mControls.size();i++){
				Control item = mControls.get(i);
				findFields(holder);
				item.Handler.ToControl(mView.get(i), object);
			}
		} catch (Exception e) {
